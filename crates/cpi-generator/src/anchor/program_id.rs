...
pub fn gen_program_id(idl: &Idl) -> proc_macro2::TokenStream {
    ...
    quote! {
        ...
        impl ProgramId for #ident {
            const ID: typhoon_program::pubkey::Pubkey = typhoon_program::pubkey_from_array([#(#id_array),*]);
        }
    }
}
    ...
    fn test_gen_program_id() {
        ...
        let expected = quote! {
            ...
            impl ProgramId for TestProgram {
                const ID: typhoon_program::pubkey::Pubkey = typhoon_program::pubkey_from_array([218u8 , 7u8 , 92u8 , 178u8 , 255u8 , 94u8 , 198u8 , 129u8 , 118u8 , 19u8 , 222u8 , 83u8 , 11u8 , 105u8 , 42u8 , 135u8 , 53u8 , 71u8 , 119u8 , 105u8 , 218u8 , 71u8 , 67u8 , 12u8 , 189u8 , 129u8 , 84u8 , 51u8 , 92u8 , 74u8 , 131u8 , 39u8]);
            }
        }.to_string();
